...
public class CategoryService {
    ...
    @Transactional public CategoryDto create(CategoryWrite body) { String slug = normalizeSlug(body.slug(), 120); ensureSlugFree(slug, null); return mapper.category(categories.save(new Category(body.name().trim(), slug, trim(body.description()), value(body.sortOrder()), value(body.isActive(), true)))); }
    @Transactional public CategoryDto update(long id, CategoryWrite body) { Category c = get(id); String slug = normalizeSlug(body.slug(), 120); ensureSlugFree(slug, id); c.setSlug(slug); c.setDescription(trim(body.description())); c.setSortOrder(value(body.sortOrder())); c.setActive(value(body.isActive(), c.isActive())); c.setName(body.name().trim()); return mapper.category(categories.save(c)); }
    ...
    static String normalizeSlug(String value, int maxLength) {
        ...
        if (normalized == null || normalized.length() > maxLength || !URL_SAFE_SLUG.matcher(normalized).matches()) {
            throw new BusinessException(HttpStatus.UNPROCESSABLE_CONTENT, "VALIDATION_ERROR", "Slug must be URL-safe");
        }
        return normalized;
    }
