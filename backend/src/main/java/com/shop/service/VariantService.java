...
public class VariantService {
    ...
    private void validate(SaleType type, VariantWrite b) { if(b.minQuantity().scale()>2||b.quantityStep().scale()>2||b.minQuantity().signum()<=0||b.quantityStep().signum()<=0||b.minQuantity().compareTo(new BigDecimal("99999999.99"))>0||b.quantityStep().compareTo(new BigDecimal("99999999.99"))>0) throw new BusinessException(HttpStatus.UNPROCESSABLE_CONTENT,"VALIDATION_ERROR","Invalid quantity rule"); if(type==SaleType.FIXED_PRICE && (b.price()==null || b.price()<0 || b.price()>9_000_000_000_000L) || type==SaleType.QUOTE && b.price()!=null) throw new BusinessException(HttpStatus.UNPROCESSABLE_CONTENT,"VALIDATION_ERROR","Price is incompatible with sale type"); if(type==SaleType.FIXED_PRICE && (notInteger(b.price(),b.minQuantity())||notInteger(b.price(),b.quantityStep()))) throw new BusinessException(HttpStatus.UNPROCESSABLE_CONTENT,"VALIDATION_ERROR","Price must produce whole VND"); }
