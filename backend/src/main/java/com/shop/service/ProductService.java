...
public class ProductService {
    ...
    @Transactional(readOnly = true) public java.util.List<ProductDto> list() { return products.findAllByOrderBySortOrderAscIdAsc().stream().map(p -> mapper.product(p, variants.findByProductIdOrderBySortOrderAscIdAsc(p.getId()))).toList(); }
    ...
    @Transactional public ProductDto create(ProductWrite b) { Category c=category(b.categoryId()); String slug=CategoryService.slug(b.slug()); unique(slug,null); Product p=new Product(c,b.name().trim(),slug,CategoryService.trim(b.shortDescription()),CategoryService.trim(b.description()),CategoryService.trim(b.thumbnailUrl()),b.saleType(),status(b.status()),CategoryService.value(b.sortOrder())); return mapper.product(products.save(p), java.util.List.of()); }
    @Transactional public ProductDto update(long id, ProductWrite b) { Product p=get(id); Category c=category(b.categoryId()); String slug=CategoryService.slug(b.slug()); unique(slug,id); if (b.saleType()!=p.getSaleType()) throw new BusinessException(HttpStatus.UNPROCESSABLE_CONTENT,"VALIDATION_ERROR","saleType cannot be changed"); p.setCategory(c);p.setName(b.name().trim());p.setShortDescription(CategoryService.trim(b.shortDescription()));p.setDescription(CategoryService.trim(b.description()));p.setThumbnailUrl(CategoryService.trim(b.thumbnailUrl()));p.setStatus(status(b.status()));p.setSortOrder(CategoryService.value(b.sortOrder())); return mapper.product(products.save(p),variants.findByProductIdOrderBySortOrderAscIdAsc(id)); }
